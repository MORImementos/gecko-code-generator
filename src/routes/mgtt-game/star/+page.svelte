            ...
            {#each row.slice(2, row.length) as column, cIndex}
              ...
              {#if changedArray[rIndex + 2][cIndex + 2] !== true}
                ...
                {#if cIndex === 0}
                  ...
                  <input class="variant-ghost-surface input text-center" bind:value={$mgttInGameWrite[rIndex + 2][cIndex + 2]} type="number" min="5" max="500" step="1" on:input={() => updateArray(rIndex + 2, cIndex + 2, $mgttInGameWrite[rIndex + 2][cIndex + 2])} />
